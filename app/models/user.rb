class User < ApplicationRecord
  ...
  validates :last_name, :first_name, presence: true, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: '全角文字である必要があります' }

  # お名前カナ(全角)が必須であること、全角（カタカナ）での入力が必須であること
  validates :last_name_kana, :first_name_kana, presence: true, format: { with: /\A[ァ-ヶー－]+\z/, message: '全角カタカナである必要があります' }
